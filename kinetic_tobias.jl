...
LX = zeros(ComplexF64,(Nspec,Nspec))
LY = zeros(ComplexF64,(Nspec,Nspec))
...
for m1=-l1:l1
	...
	for m2=-l2:l2
		...
		if n1 == n2 && l1 == l2 && m1 == m2+1
			Cplus=sqrt(l1*(l1+1)-m2*(m2+1))
			LX[i1,i1]=0.5*Cplus
			LY[i1,i1]=-0.5im*Cplus
		end
		if n1 == n2 && l1 == l2 && m1 == m2-1
			Cminus=sqrt(l1*(l1+1)-m2*(m2-1))
			LX[i1,i1]=0.5*Cminus
			LY[i1,i1]=0.5im*Cminus
		end
	end
	end
	end
