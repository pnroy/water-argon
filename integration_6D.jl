...
export integration,transformation_forward_HO,transformation_forward_Wigner,transformation_backward_HO,transformation_backward_Wigner

###############################################################################################################
function integration(Property,nmax,NR,Ntheta,Nphi,jmax,Nalpha,Nm,Nk,omega,mass,Ntrans,Nrot,eq_struct,error)
...
close(log_file)
C,Vtrans,Vrot = decomposition(Property,error)
Nprod = length(C)
